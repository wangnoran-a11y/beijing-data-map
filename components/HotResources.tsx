...
export default function HotResources() {
  ...
  return (
    <section className="bg-white px-5 py-5 sm:px-8 lg:px-6 xl:px-10 2xl:px-14">
      <div className="mx-auto w-full max-w-[1720px]">
        ...
        <div className="overflow-hidden rounded-[32px] border border-red-100 bg-gradient-to-br from-red-50/50 via-white to-gray-50 p-3 shadow-sm sm:p-4 lg:p-5">
          <div className="grid grid-cols-1 gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
            <div className="min-w-0">
              <div className="mb-5 grid auto-rows-fr grid-cols-1 gap-5 md:grid-cols-2 2xl:grid-cols-3">
                {topThree.map((item) => (
                  <Link
                    href="/data-resources"
                    key={item.title}
                    className={`group relative flex h-full flex-col overflow-hidden rounded-[26px] p-5 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl ${
                      item.rank === 1 ? "bg-[#111827] text-white" : "border border-red-100 bg-white text-gray-950"
                    }`}
                  >
                    <div className="absolute right-0 top-0 h-28 w-28 rounded-full bg-[#C41E3A]/20 blur-3xl" />

                    <div className="relative mb-5 flex items-center justify-between">
                      <div className={`flex h-12 w-12 items-center justify-center rounded-2xl text-lg font-black ${item.rank === 1 ? "bg-[#C41E3A] text-white" : "bg-red-50 text-[#C41E3A]"}`}>
                        {item.rank === 1 ? <Crown className="h-6 w-6" /> : item.rank}
                      </div>

                      <span className={`flex items-center gap-1 rounded-full px-3 py-1 text-xs font-bold ${item.rank === 1 ? "bg-white/10 text-red-100" : "bg-red-50 text-[#C41E3A]"}`}>
                    ...
                    <div className="relative flex flex-1 flex-col">
                      <p className={`mb-3 inline-flex w-fit rounded-full px-3 py-1 text-xs font-semibold ${item.rank === 1 ? "bg-white/10 text-white/80" : "bg-gray-50 text-gray-500"}`}>
                        {item.scene}
                      </p>
                      ...
                      <p className={`mt-3 text-sm leading-6 ${item.rank === 1 ? "text-white/60" : "text-gray-500"}`}>
                        {item.desc}
                      </p>
                      ...
                      <div className="mt-auto flex items-center justify-between pt-5">
                        ...
                        <span className={`flex items-center gap-1 text-xs ${item.rank === 1 ? "text-white/50" : "text-gray-400"}`}>
              ...
              <div className="grid auto-rows-fr grid-cols-1 gap-4 md:grid-cols-2 2xl:grid-cols-3">
                {rest.map((item) => (
                  <Link href="/data-resources" key={item.title} className="group flex h-full flex-col rounded-[24px] border border-gray-100 bg-white p-5 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-red-100 hover:shadow-lg">
                    <div className="mb-4 flex items-center justify-between">
                      ...
                      <span className="text-sm font-black text-[#C41E3A]">{item.hot}</span>
                    </div>
                    ...
                    <div className="mt-auto pt-5">
                      <div className="h-2 overflow-hidden rounded-full bg-gray-100">
                        <div className="h-full rounded-full bg-gradient-to-r from-[#C41E3A] to-[#D4A574]" style={{ width: `${item.hot}%` }} />
                      </div>
              ...
              >
                <div className="rounded-2xl bg-white/10 p-4">
                  <div className="mb-2 flex items-center gap-2">
                    ...
                    <span className="text-sm font-bold text-white">榜单生成规则</span>
                  </div>
            ...
            <div className="flex h-full flex-col gap-5">
              <div className="rounded-[26px] border border-gray-100 bg-white p-5 shadow-sm">
                ...
                <div className="space-y-3">
                  {resources.slice(0, 5).map((item) => (
                    <div key={item.title} className="flex items-center gap-3 rounded-2xl bg-gray-50 px-4 py-3">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="mt-1 text-xs text-gray-400">{item.update}</p>
                      </div>
              ...
              <div className="grid grid-cols-2 gap-3 xl:grid-cols-2">
                {stats.map((item) => {
                  ...
                  return (
                    <div key={item.label} className="min-h-[140px] rounded-[20px] border border-gray-100 bg-white p-3.5 shadow-sm">
